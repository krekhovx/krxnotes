...
исследовал и решил это записать (субъективный опыт). Репозиторий может быть
полезен русскоязычным **Linux** инженерам. Я использую последний стабильный
выпуск Debian GNU/Linux с архитектурой amd64.

Эти заметки я часто перечитываю перед собеседованиями, чтобы освежить память.
Оформлению коммитов не уделяю особого внимания, поскольку информация регулярно
обновляется и дополняется. В файлах используется не только русский, но и немного
английского языка.

У меня есть Telegram-[канал](https://t.me/krxnotes), где выборочно публикуются
некоторые заметки в современном формате.

Спасибо всем, кто поддержал репозиторий звездой :star:
...
Книги (моя небольшая мини-библиотека):
```
books/
```
...
темы. Описание темы начинается с ```---```. Обычно в ```vim``` я делаю:
```
/--- Название темы
```
и перехожу к описанию темы.

Можно вывести все темы из оглавлений, указанные в заметках. Это позволит лучше
